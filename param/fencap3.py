...
def writeData():
    ...
    try:
        if os.path.getsize('./param/Main3.txt'):
            print("+ File 'Main3.txt' exist !")
    except FileNotFoundError as info:
        print("The file : 'Main3.txt' doesn't exist !", info)
        print("File 'Main3.txt' created !")
    finally:
        with open('./param/Main3.txt', 'a+') as file:
            file.write(str("Date: "))
    ...
    try:
        if os.path.getsize('./param/aspifile3/tensor.json'):
            ...
            with open('./param/aspifile3/tensor.json', 'r') as datafile:
            ...
            if textTa.get() == "":
                ...
                with open('./param/aspifile3/tensor.json', 'w') as datafile2:
        ...
        if textTa.get() == "":
            ...
            print("---Ok VALEUR 'Tension' entrée---")
            with open('./param/aspifile3/tensor.json', 'w') as datafile:
...
def mainRead():
    subprocess.call('./param/main_read3.py')
...
def delMain():
    ...
    Main_MsgBox = messagebox.askquestion("Confirm","Are you sure ?\n"
        "It will delete Main3.txt with all data !!!")
    if Main_MsgBox == 'yes':
        try:
            if os.path.getsize('./param/Main3.txt'):
                os.remove('./param/Main3.txt')
                label['text'] = "File Main3.txt has been deleted !"
                print("+ File Main3.txt has been deleted !")
...
def delTA():
    ...
    try:
        if os.path.getsize('./param/aspifile3/tensor.json'):
            with open('./param/aspifile3/tensor.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("Last value of TA deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/tensor.json', 'w') as file:
...
def updateData():
    ...
    for i in listeDate:
        if textDate.get() == i:
            ...
            shutil.copy('./param/Main3.txt', './Backup/BackupMain3.txt')
            with open('./param/Main3.txt', 'w'): pass
        else:
            pass
