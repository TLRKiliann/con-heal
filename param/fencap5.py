...
def writeData():
    ...
    try:
        if os.path.getsize('./param/Main5.txt'):
            print("+ File 'Main5.txt' exist !")
    except FileNotFoundError as info:
        print("The file : 'Main5.txt' doesn't exist !", info)
        print("File 'Main5.txt' created !")
    finally:
        with open('./param/Main5.txt', 'a+') as file:
            file.write(str("Date: "))
    ...
    try:
        if os.path.getsize('./param/aspifile5/tensor.json'):
            ...
            with open('./param/aspifile5/tensor.json', 'r') as datafile:
            ...
            if textTa.get() == "":
                ...
                with open('./param/aspifile5/tensor.json', 'w') as datafile2:
        ...
        if textTa.get() == "":
            ...
            print("---Ok VALEUR 'Tension' entrée---")
            with open('./param/aspifile5/tensor.json', 'w') as datafile:
...
def mainRead():
    subprocess.call('./param/main_read5.py')
...
def delMain():
    ...
    Main_MsgBox = messagebox.askquestion("Confirm","Are you sure ?\n"
        "It will delete Main5.txt with all data !!!")
    if Main_MsgBox == 'yes':
        try:
            if os.path.getsize('./param/Main5.txt'):
                os.remove('./param/Main5.txt')
                label['text'] = "File Main5.txt has been deleted !"
                print("+ File Main5.txt has been deleted !")
...
def delTA():
    ...
    try:
        if os.path.getsize('./param/aspifile5/tensor.json'):
            with open('./param/aspifile5/tensor.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("Last value of TA deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile5/tensor.json', 'w') as file:
...
def updateData():
    ...
    for i in listeDate:
        if textDate.get() == i:
            ...
            shutil.copy('./param/Main5.txt', './Backup/BackupMain5.txt')
            with open('./param/Main5.txt', 'w'): pass
        else:
            pass
