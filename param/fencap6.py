...
def writeData():
    ...
    try:
        if os.path.getsize('./param/Main6.txt'):
            print("+ File 'Main6.txt' exist !")
    except FileNotFoundError as info:
        print("The file : 'Main6.txt' doesn't exist !", info)
        print("File 'Main6.txt' created !")
    finally:
        with open('./param/Main6.txt', 'a+') as file:
            file.write(str("Date: "))
    ...
    try:
        if os.path.getsize('./param/aspifile6/tensor.json'):
            ...
            with open('./param/aspifile6/tensor.json', 'r') as datafile:
            ...
            if textTa.get() == "":
                ...
                with open('./param/aspifile6/tensor.json', 'w') as datafile2:
        ...
        if textTa.get() == "":
            ...
            print("---Ok VALEUR 'Tension' entrée---")
            with open('./param/aspifile6/tensor.json', 'w') as datafile:
...
def mainRead():
    subprocess.call('./param/main_read6.py')
...
def delMain():
    ...
    Main_MsgBox = messagebox.askquestion("Confirm","Are you sure ?\n"
        "It will delete Main6.txt with all data !!!")
    if Main_MsgBox == 'yes':
        try:
            if os.path.getsize('./param/Main6.txt'):
                os.remove('./param/Main6.txt')
                label['text'] = "File Main6.txt has been deleted !"
                print("+ File Main6.txt has been deleted !")
...
def delTA():
    ...
    try:
        if os.path.getsize('./param/aspifile6/tensor.json'):
            with open('./param/aspifile6/tensor.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("Last value of TA deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile6/tensor.json', 'w') as file:
...
def updateData():
    ...
    for i in listeDate:
        if textDate.get() == i:
            ...
            shutil.copy('./param/Main6.txt', './Backup/BackupMain6.txt')
            with open('./param/Main6.txt', 'w'): pass
        else:
            pass
